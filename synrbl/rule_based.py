...
class RuleBasedMethod:
    ...
    def run(self, reactions, stats=None):
        ...
        id_mapping = {str(orig_r[self.id_col]): orig_r for orig_r in reactions}
        
        for r in certain_reactions:
            _id_str = str(r[self.id_col])
            if _id_str in id_mapping:
                id_mapping[_id_str][self.output_col] = r["new_reaction"]  # type: ignore
